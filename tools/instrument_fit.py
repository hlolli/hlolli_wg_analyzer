...
ANALYZER_EVIDENCE_SHA256 = (
    "d55db1de35b076165239a0b18b4f9c83a17014280da19efc3de94e91cce51092"
)
...
def fit_manifest(path: Path, source: Optional[bytes] = None) -> dict[str, Any]:
    ...
    for index, row in enumerate(objectives):
        ...
        if kind not in ("experiment-gap", "body-envelope", "passive-decay",
                        ...
                        "checked-note-harmonic-decay", "note-phase"):
        ...
        if kind == "note-phase":
            ...
            for field in ("reference_span", "model_span"):
                span = row.get(field)
                if (type(span) is not list or len(span) != 2 or
                        any(type(v) is not int for v in span) or
                        not 0 <= span[0] < span[1] <= 2000000000):
                    raise FitError("note-phase objective has invalid " + field)
        if version == 2:
            objective_keys = {
...
def run_note_phase(
    ...
    try:
        ...
        reports = {
            name: checks.note_phases(name, *objective[name + "_span"])
    ...
    for name, report in reports.items():
        phase = report.phases[objective["phase"]]
        metric = phase["metrics"][objective["metric"]]
        if phase["status"] != "valid" or metric["status"] != "valid":
            raise FitError("{} note-phase {} is not usable: {}/{}".format(
                name, objective["phase"], phase["status"], metric["status"]))
        values[name + "_value"] = metric["value"]
        values[name + "_start_sample"] = phase["start_sample"]
