...
int main(int argc, char **argv)
{
    ...
    CHECK(render(&bundle, 1U, &options, text) == 0);
    CHECK(memcmp(text, "MThd\0\0\0\x06\0\x01\0\x05\x7f\xff", 14U) == 0);
    options.tempo_bpm = 120U;
    CHECK(render(&bundle, 1U, &options, text) != 0);
    options.tempo_bpm = 0U;
    values[4].number = values[0].number;
    CHECK(render(&bundle, 1U, &options, text) != 0);
    fixture(&bundle, &audio, events, values);
    audio.format.frames = UINT64_C(1000000000);
    audio.format.data_bytes = audio.format.frames*2U;
    audio.format.duration_seconds = (double)audio.format.frames/8000.0;
    CHECK(render(&bundle, 1U, &options, text) != 0);
    fixture(&bundle, &audio, events, values);
    values[0].number = 1.0;
    CHECK(render(&bundle, 1U, &options, text) != 0);
