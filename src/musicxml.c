...
typedef struct XMLReader {
    ...
    char *error;
    size_t error_size;
    const char *part_id, *measure_number;
} XMLReader;

static int fail(XMLReader *r, const char *message)
{
    if (r->part_id != NULL && r->measure_number != NULL) {
        hwa_set_error(r->error, r->error_size, "MusicXML at byte %zu (part %s, measure %s): %s",
                      r->pos, r->part_id, r->measure_number, message);
    } else {
...
static int is_xml_space(unsigned char c)
{
    return c == ' ' || c == '\t' || c == '\r' || c == '\n';
}
...
{
    while (r->pos < r->size && is_xml_space(r->data[r->pos])) {
        r->pos++;
    }
}

static int starts_with(const XMLReader *r, const char *s)
{
    size_t n = strlen(s);
    return n <= r->size - r->pos && memcmp(r->data + r->pos, s, n) == 0;
}

static int is_xml_codepoint(uint32_t c)
{
    return c == 9U || c == 10U || c == 13U || (c >= 32U && c <= 0xd7ffU) ||
           (c >= 0xe000U && c <= 0xfffdU) || (c >= 0x10000U && c <= 0x10ffffU);
}

static int validate_utf8(const unsigned char *s, size_t n)
{
    size_t i = 0U;
    while (i < n) {
        ...
        while (extra != 0U) {
            if (i == n || (s[i] & 0xc0U) != 0x80U) {
                return -1;
            }
            c = (c << 6U) | (uint32_t)(s[i++] & 63U);
            extra--;
        }
        if (c < minimum || !is_xml_codepoint(c)) {
            return -1;
        }
    }
    return 0;
...
static size_t encode_utf8(char *out, uint32_t c)
{
    if (c < 0x80U) {
        out[0] = (char)c;
        return 1U;
    ...
    if (c < 0x800U) {
        ...
        out[1] = (char)(0x80U | (c & 63U));
        return 2U;
    }
    if (c < 0x10000U) {
        out[0] = (char)(0xe0U | (c >> 12U));
        out[1] = (char)(0x80U | ((c >> 6U) & 63U));
        out[2] = (char)(0x80U | (c & 63U));
        return 3U;
    }
    out[0] = (char)(0xf0U | (c >> 18U));
    out[1] = (char)(0x80U | ((c >> 12U) & 63U));
    out[2] = (char)(0x80U | ((c >> 6U) & 63U));
    out[3] = (char)(0x80U | (c & 63U));
    return 4U;
}

static const char *copy_xml_text(XMLReader *r, size_t begin, size_t end, int entities)
{
    char *out;
    size_t length = 0U, i = begin;
    if (end - begin >= r->strings_size - r->strings_used) {
        (void)fail(r, "string storage limit exceeded");
        return NULL;
    }
    ...
    while (i < end) {
        ...
        if (c == '&' && entities) {
            size_t start = i;
            uint32_t value = 0U;
            while (i < end && r->data[i] != ';' && i - start < 32U) {
                i++;
            }
            if (i == end || r->data[i] != ';') {
                goto bad_entity;
            }
            if (i - start == 3U && memcmp(r->data + start, "amp", 3U) == 0) {
                value = '&';
            } else if (i - start == 2U && memcmp(r->data + start, "lt", 2U) == 0) {
                value = '<';
            } else if (i - start == 2U && memcmp(r->data + start, "gt", 2U) == 0) {
                value = '>';
            } else if (i - start == 4U && memcmp(r->data + start, "quot", 4U) == 0) {
                value = '"';
            } else if (i - start == 4U && memcmp(r->data + start, "apos", 4U) == 0) {
                value = '\'';
            } else if (r->data[start] == '#') {
                unsigned base = 10U;
                size_t digit = start + 1U;
                if (digit < i && r->data[digit] == 'x') {
                    base = 16U;
                    digit++;
                }
                if (digit == i) {
                ...
                for (; digit < i; digit++) {
                    ...
                    if (d >= base || value > (0x10ffffU - d) / base) {
                        goto bad_entity;
                    }
                    value = value * base + d;
                }
            ...
            if (!is_xml_codepoint(value)) {
                goto bad_entity;
            }
            length += encode_utf8(out + length, value);
            i++;
        } else {
            if (c == '\r') {
                c = '\n';
...
static size_t append_xml_node(XMLReader *r, size_t parent, int attribute)
{
    size_t index;
    if (r->count == r->limits.max_nodes) {
        (void)fail(r, "XML node limit exceeded");
        return 0U;
    }
    if (r->count + 1U >= r->capacity) {
        size_t cap = r->capacity == 0U ? 64U : r->capacity * 2U;
        XMLNode *p;
        if (cap < r->capacity || cap > r->limits.max_nodes + 1U) {
            cap = r->limits.max_nodes + 1U;
        }
    ...
    if (parent != 0U) {
        if (r->nodes[parent].last != 0U) {
            r->nodes[r->nodes[parent].last].next = index;
        } else {
            r->nodes[parent].first = index;
        }
...
static const char *read_xml_name(XMLReader *r)
{
    size_t start = r->pos;
    while (r->pos < r->size) {
        ...
        if (!letter &&
            !(r->pos > start && ((c >= '0' && c <= '9') || c == '-' || c == '.' || c == ':'))) {
            break;
        }
        r->pos++;
    }
    if (r->pos == start) {
        (void)fail(r, "invalid XML name");
        return NULL;
    }
    return copy_xml_text(r, start, r->pos, 0);
}
...
static int skip_misc(XMLReader *r)
{
    if (starts_with(r, "<!--")) {
        r->pos += 4U;
        while (r->pos < r->size && !starts_with(r, "--")) {
            r->pos++;
        }
        if (!starts_with(r, "-->")) {
            return fail(r, "invalid XML comment");
        }
        r->pos += 3U;
        return 1;
    }
    if (starts_with(r, "<?")) {
        ...
        const char *target;
        r->pos += 2U;
        target = read_xml_name(r);
        if (target == NULL) {
            return -1;
        }
        if ((target[0] == 'x' || target[0] == 'X') && (target[1] == 'm' || target[1] == 'M') &&
            ...
            for (;;) {
                ...
                if (starts_with(r, "?>")) {
                    if (stage == 0) {
                        return fail(r, "missing XML version");
                    }
                    r->pos += 2U;
                    return 1;
                }
                if (before == r->pos) {
                    return fail(r, "invalid XML declaration separator");
                }
                key = read_xml_name(r);
                if (key == NULL) {
                    return -1;
                ...
                skip_space(r);
                if (!starts_with(r, "=")) {
                    return fail(r, "invalid XML declaration value");
                }
                r->pos++;
                skip_space(r);
                ...
                quote = r->data[r->pos++];
                start = r->pos;
                while (r->pos < r->size && r->data[r->pos] != quote) {
                    r->pos++;
                }
                if (r->pos == r->size) {
                    return fail(r, "unterminated XML declaration");
                }
                value = copy_xml_text(r, start, r->pos, 0);
                r->pos++;
                if (value == NULL) {
                    return -1;
                }
                if (strcmp(key, "version") == 0 && stage == 0 && strcmp(value, "1.0") == 0) {
                    stage = 1;
                } else if (strcmp(key, "encoding") == 0 && stage == 1) {
                    if (strlen(value) != 5U || (value[0] != 'U' && value[0] != 'u') ||
                        (value[1] != 'T' && value[1] != 't') ||
                        (value[2] != 'F' && value[2] != 'f') || value[3] != '-' ||
                        value[4] != '8') {
                        return fail(r, "only UTF-8 XML is supported");
                    }
                    stage = 2;
                    ...
                    return fail(r, "invalid or unsupported XML declaration");
                }
            }
        }
        if (!starts_with(r, "?>") && (r->pos == r->size || !is_xml_space(r->data[r->pos]))) {
        ...
        if (!starts_with(r, "?>")) {
            return fail(r, "unterminated processing instruction");
        }
        r->pos += 2U;
        return 1;
    }
...
static size_t element(XMLReader *r, size_t parent, unsigned depth)
{
    ...
    const char *tag;
    if (depth > 64U) {
        (void)fail(r, "XML depth limit exceeded");
        return 0U;
    }
    r->pos++;
    tag = read_xml_name(r);
    if (tag == NULL || (index = append_xml_node(r, parent, 0)) == 0U) {
        return 0U;
    }
    r->nodes[index].name = tag;
    r->nodes[index].start = begin;
    for (;;) {
        ...
        if (before == r->pos) {
            (void)fail(r, "expected attribute separator");
            return 0U;
        }
        if (++attribute_count > 128U) {
            (void)fail(r, "attribute limit exceeded");
            return 0U;
        }
        key = read_xml_name(r);
        if (key == NULL) {
        ...
        for (scan = r->nodes[index].first; scan != 0U; scan = r->nodes[scan].next) {
            if (strcmp(r->nodes[scan].name, key) == 0) {
                (void)fail(r, "duplicate XML attribute");
                return 0U;
            }
        }
        skip_space(r);
        if (!starts_with(r, "=")) {
            (void)fail(r, "expected attribute value");
            return 0U;
        }
        r->pos++;
        skip_space(r);
        if (r->pos == r->size || (r->data[r->pos] != '\'' && r->data[r->pos] != '"')) {
            (void)fail(r, "expected quoted attribute");
            return 0U;
        }
        quote = r->data[r->pos++];
        start = r->pos;
        while (r->pos < r->size && r->data[r->pos] != quote && r->data[r->pos] != '<') {
            r->pos++;
        }
        if (r->pos == r->size || r->data[r->pos] != quote) {
            (void)fail(r, "invalid attribute");
            return 0U;
        }
        value = copy_xml_text(r, start, r->pos, 1);
        r->pos++;
        if (value == NULL || (attr = append_xml_node(r, index, 1)) == 0U) {
            return 0U;
        }
        r->nodes[attr].name = key;
        r->nodes[attr].text = value;
    }
    while (r->pos < r->size) {
        int misc;
        if (starts_with(r, "</")) {
            const char *close;
            r->pos += 2U;
            close = read_xml_name(r);
            skip_space(r);
            if (close == NULL || strcmp(close, tag) != 0 || !starts_with(r, ">")) {
                (void)fail(r, "mismatched closing element");
                return 0U;
            }
            r->pos++;
            r->nodes[index].end = r->pos;
        ...
        if (starts_with(r, "<![CDATA[")) {
            ...
            if (!starts_with(r, "]]>")) {
                (void)fail(r, "unterminated CDATA");
                return 0U;
            }
            value = copy_xml_text(r, begin, r->pos, 0);
            r->pos += 3U;
            if (value == NULL) {
                return 0U;
            }
            if (r->nodes[index].text[0] != '\0') {
                (void)fail(r, "split text is unsupported");
                return 0U;
            }
            r->nodes[index].text = value;
        } else if (starts_with(r, "<")) {
            if (element(r, index, depth + 1U) == 0U) {
                return 0U;
            }
        } else {
            const char *value;
            ...
            while (r->pos < r->size && r->data[r->pos] != '<') {
                if (starts_with(r, "]]>")) {
                    (void)fail(r, "invalid text delimiter");
                    return 0U;
                }
                r->pos++;
            }
            value = copy_xml_text(r, begin, r->pos, 1);
            if (value == NULL) {
                return 0U;
            }
            while (*value != '\0' && is_xml_space((unsigned char)*value)) {
                value++;
            }
            if (*value != '\0') {
                if (r->nodes[index].text[0] != '\0') {
    ...
    (void)fail(r, "unterminated element");
    return 0U;
}
...
static int decimal(const char *s, double *out)
{
    ...
    while (is_xml_space((unsigned char)*s)) {
        s++;
    }
    if (*s == '-' || *s == '+') {
        negative = *s == '-';
    ...
    while (*s >= '0' && *s <= '9') {
        number = number * 10.0 + (double)(*s++ - '0');
        digits = 1;
    }
    if (*s == '.') {
        s++;
        while (*s >= '0' && *s <= '9') {
            number += (double)(*s++ - '0') * scale;
            scale *= 0.1;
            digits = 1;
        }
    }
    ...
    if (*s != '\0' || !digits || !isfinite(number) || number > 1e12) {
        return -1;
    }
...
static int number(XMLReader *r, size_t parent, const char *key, double *out)
{
    ...
    if (n == 0U) {
        return fail(r, "missing numeric element");
    }
    ...
    for (i = r->nodes[n].first; i != 0U; i = r->nodes[i].next) {
        if (!r->nodes[i].attribute) {
            return fail(r, "nested numeric content");
        }
    }
...
static int append(XMLReader *r, const HWAMusicXMLEvent *event, size_t measure)
{
    ...
    if (count == r->limits.max_events) {
        return fail(r, "event limit exceeded");
    }
    if (count == r->event_capacity) {
        size_t cap = count == 0U ? 64U : count * 2U;
        HWAMusicXMLEvent *events;
        size_t *indices;
        if (cap < count || cap > r->limits.max_events) {
            cap = r->limits.max_events;
        }
        ...
        r->event_measures = indices;
        r->event_capacity = cap;
    }
...
static int measure_slot(XMLReader *r, size_t index)
{
    if (index == r->measure_count) {
        if (index == r->measure_capacity) {
            ...
            XMLMeasure *p;
            if (cap < index || cap > r->limits.max_nodes) {
                cap = r->limits.max_nodes;
            }
            if (cap <= index) {
                return fail(r, "measure limit exceeded");
            }
...
{
    static const char *const names[] = {"long",   "breve", "whole", "half", "quarter",
                         ...
                         HWAMusicXMLEvent *event)
{
    ...
    if (written == 0.0 || floor(divisions) != divisions || floor(ticks) != ticks) {
        return 0;
    }
    dot = written * 0.5;
    ...
    if (!isfinite(expected) || expected <= 0.0 || floor(expected + 0.5) != ticks ||
        fabs(expected - ticks) <= 1e-9) {
        return 0;
    }
    event->duration_beats = written;
    event->interpretation |= HWA_MUSICXML_TUPLET_REPAIR;
...
static int meter(XMLReader *r, size_t time, double *length)
{
    ...
    if (child(r, time, "senza-misura") != 0U) {
        *length = 0.0;
        return 0;
    }
    for (i = r->nodes[time].first; i != 0U; i = r->nodes[i].next) {
        ...
        if (strcmp(key, "beats") == 0) {
            ...
            if (pending != 0.0) {
                return fail(r, "unpaired meter beats");
            }
            do {
                double n = 0.0;
                int digits = 0;
                ...
                if (!digits || n < 1.0 || n > 1024.0) {
                    return fail(r, "invalid meter beats");
                }
                pending += n;
                while (is_xml_space((unsigned char)*s)) {
                    s++;
                ...
                if (*s++ != '+') {
                    return fail(r, "unsupported meter");
                }
            } while (1);
            ...
            total += pending * 4.0 / denominator;
            pending = 0.0;
        }
    }
    if (pending != 0.0 || total <= 0.0) {
        return fail(r, "incomplete meter");
    }
    *length = total;
    return 0;
}
...
{
    static const char *const forbidden[] = {"measure-repeat", "beat-repeat",   "multiple-rest",
                                            "swing",          "unpitched",
                                            "part-link",      "concert-score", "for-part"};
    size_t i, j;
    for (i = 1U; i <= r->count; i++) {
        ...
        if (strcmp(r->nodes[i].name, "repeat") == 0 &&
            strcmp(attribute(r, i, "after-jump"), "yes") == 0) {
...
static int mark_event(XMLReader *r, const HWAMusicXMLEvent *parent, size_t n, size_t measure)
{
    ...
    event.kind = HWA_MUSICXML_MARK;
    event.duration_beats = 0.0;
    event.written_duration_beats = 0.0;
    event.velocity_valid = 0;
    event.interpretation &= ~HWA_MUSICXML_PLAYBACK_DATA;
    if (parent->kind == HWA_MUSICXML_CUE) {
        event.interpretation |= HWA_MUSICXML_CUE_NOTATION;
    }
    event.mark = r->nodes[n].name;
    event.mark_tag = r->nodes[n].name;
    event.mark_text = r->nodes[n].text;
    event.mark_type = attribute(r, n, "type");
...
static int controls(XMLReader *r, const HWAMusicXMLEvent *parent, double playback_start,
                    size_t n, size_t measure)
{
    ...
    event.start_beats = event.written_start_beats = playback_start;
    for (i = 0U; i < 3U; i++) {
        ...
        if (strcmp(s, "yes") == 0) {
            ...
            offset = 0.0;
        } else if (decimal(s, &offset) != 0) {
            return fail(r, "invalid pedal value");
        }
        if (offset < 0.0 || offset > 100.0) {
            return fail(r, "pedal percentage outside 0..100");
        }
        event.kind = HWA_MUSICXML_CONTROL;
        event.controller = cc[i];
        event.value = offset * 1.27;
        event.mark = pedals[i];
        event.interpretation = HWA_MUSICXML_PLAYBACK_DATA;
        event.sequence = ++sequence;
        if (append(r, &event, measure) != 0) {
            return -1;
        }
    }
    if (*attribute(r, sound, "dynamics") != '\0') {
        if (numeric_attribute(r, sound, "dynamics", 0.0, &offset) != 0 || offset < 0.0) {
            return fail(r, "invalid dynamics");
        }
        event.kind = HWA_MUSICXML_MARK;
        event.controller = 0U;
        event.value = fmin(127.0, offset * 0.9);
        event.mark = "velocity";
        event.interpretation = HWA_MUSICXML_PLAYBACK_DATA;
        event.sequence = ++sequence;
        if (append(r, &event, measure) != 0) {
    ...
    for (i = r->nodes[n].first; i != 0U; i = r->nodes[i].next) {
        size_t j;
        if (r->nodes[i].attribute || strcmp(r->nodes[i].name, "direction-type") != 0) {
            continue;
        }
        for (j = r->nodes[i].first; j != 0U; j = r->nodes[j].next) {
            ...
            if (strcmp(tag, "dynamics") == 0) {
                ...
                for (d = r->nodes[j].first; d != 0U; d = r->nodes[d].next) {
                    if (!r->nodes[d].attribute && mark_event(r, parent, d, measure) != 0) {
                        return -1;
                    }
                }
                ...
                double number;
                unsigned pedal;
                if (numeric_attribute(r, j, "number", 1.0, &number) != 0 || number < 1.0 ||
                    number > 16.0 || floor(number) != number) {
                    return fail(r, "invalid pedal number");
                }
                pedal = (unsigned)number;
                event = *parent;
                event.kind = HWA_MUSICXML_CONTROL;
                event.mark = "damper-pedal";
                if (strcmp(type, "sostenuto") == 0) {
                    r->pedal_cc[pedal] = 66U;
                } else if (strcmp(type, "start") == 0) {
                    r->pedal_cc[pedal] = 64U;
                }
                event.controller = r->pedal_cc[pedal] ? r->pedal_cc[pedal] : 64U;
                event.interpretation = HWA_MUSICXML_BASELINE_POLICY;
                if (event.controller == 66U) {
                    event.mark = "sostenuto-pedal";
                }
                if (strcmp(type, "stop") == 0) {
                    r->pedal_cc[pedal] = 0U;
                }
                if (strcmp(type, "continue") == 0 || strcmp(type, "resume") == 0 ||
                    strcmp(type, "discontinue") == 0) {
                    if (mark_event(r, parent, j, measure) != 0) {
                        return -1;
                    }
                    continue;
                }
                if (strcmp(type, "start") != 0 && strcmp(type, "sostenuto") != 0 &&
                    strcmp(type, "stop") != 0 && strcmp(type, "change") != 0) {
                    return fail(r, "invalid pedal mark type");
                }
                /* Explicit playback data wins over its displayed pedal mark. */
                if (*attribute(r, sound,
                               event.controller == 66U ? "sostenuto-pedal" : "damper-pedal") !=
                    '\0') {
                    continue;
                }
                event.value =
                    strcmp(type, "stop") == 0 || strcmp(type, "change") == 0 ? 0.0 : 127.0;
                /* Keep generated controls in source order even with an unstable qsort. */
                event.sequence = ++sequence;
                if (append(r, &event, measure) != 0) {
                    return -1;
                }
...
static int note_expression(XMLReader *r, HWAMusicXMLEvent *event, size_t n, size_t measure,
                           double divisions)
{
    ...
    if (*attribute(r, n, "dynamics") != '\0') {
        double v;
        if (numeric_attribute(r, n, "dynamics", 0.0, &v) != 0 || v < 0.0) {
            return fail(r, "invalid note dynamics");
        }
        if (event->kind != HWA_MUSICXML_CUE) {
            event->velocity = fmin(127.0, v * 0.9);
    ...
    if (grace != 0U) {
        if (numeric_attribute(r, grace, "steal-time-previous", -1.0, &event->grace_previous) != 0 ||
            numeric_attribute(r, grace, "steal-time-following", -1.0, &event->grace_following) !=
                0 ||
            numeric_attribute(r, grace, "make-time", -1.0, &event->grace_make) != 0) {
            return -1;
        }
        if (event->grace_previous < -1.0 || event->grace_previous > 100.0 ||
            event->grace_following < -1.0 || event->grace_following > 100.0 ||
            event->grace_make < -1.0) {
            return fail(r, "invalid grace timing");
        }
        if (event->grace_make >= 0.0) {
            if (divisions <= 0.0) {
                return fail(r, "grace make-time needs divisions");
            }
    ...
    for (notation = r->nodes[n].first; notation != 0U; notation = r->nodes[notation].next) {
        size_t group;
        if (r->nodes[notation].attribute || strcmp(r->nodes[notation].name, "notations") != 0) {
            continue;
        }
        for (group = r->nodes[notation].first; group != 0U; group = r->nodes[group].next) {
            size_t mark;
            const char *group_name = r->nodes[group].name;
            if (r->nodes[group].attribute) {
                continue;
            }
            if (strcmp(group_name, "articulations") != 0 && strcmp(group_name, "ornaments") != 0 &&
                strcmp(group_name, "dynamics") != 0) {
                if (strcmp(group_name, "tied") != 0 && mark_event(r, event, group, measure) != 0) {
                    return -1;
                }
                continue;
            }
            for (mark = r->nodes[group].first; mark != 0U; mark = r->nodes[mark].next) {
                ...
                if (r->nodes[mark].attribute) {
                    continue;
                }
                if (mark_event(r, event, mark, measure) != 0) {
                    return -1;
                }
                if (strcmp(group_name, "dynamics") == 0) {
                    continue;
                }
                if (strcmp(s, "staccato") == 0) {
                    event->articulations |= HWA_MUSICXML_STACCATO;
                } else if (strcmp(s, "staccatissimo") == 0) {
                    event->articulations |= HWA_MUSICXML_STACCATISSIMO;
                } else if (strcmp(s, "tenuto") == 0) {
                    event->articulations |= HWA_MUSICXML_TENUTO;
                } else if (strcmp(s, "accent") == 0) {
                    event->articulations |= HWA_MUSICXML_ACCENT;
                } else if (strcmp(s, "strong-accent") == 0) {
                    event->articulations |= HWA_MUSICXML_STRONG_ACCENT;
                } else if (strcmp(s, "trill-mark") == 0 || strcmp(s, "mordent") == 0 ||
                           strcmp(s, "inverted-mordent") == 0 || strcmp(s, "turn") == 0 ||
                           strcmp(s, "inverted-turn") == 0 || strcmp(s, "delayed-turn") == 0 ||
                           strcmp(s, "delayed-inverted-turn") == 0) {
                    if (ornament != 0U) {
                        return fail(r,
                                    "multiple simultaneous ornaments need an explicit performance");
                    }
                    ornament = mark;
                } else {
                    r->score->unrendered_marks++;
    ...
    if (!r->limits.performance || event->kind != HWA_MUSICXML_NOTE) {
        return append(r, event, measure);
    }
    if (event->articulations & HWA_MUSICXML_STACCATISSIMO) {
        gate = 0.25;
    } else if (event->articulations & HWA_MUSICXML_STACCATO) {
        gate = r->limits.staccato_ratio;
    }
    if (gate != 1.0) {
        event->interpretation |= HWA_MUSICXML_BASELINE_POLICY;
    }
    if (ornament != 0U) {
        const char *s = r->nodes[ornament].name, *step = attribute(r, ornament, "trill-step"),
                   *start = attribute(r, ornament, "start-note");
        double count = strcmp(s, "trill-mark") == 0
                           ? fmax(3.0, ceil(event->duration_beats * r->limits.trill_notes_per_beat))
                       : strstr(s, "turn") != NULL ? 4.0
                                                   : 3.0;
        double interval = 2.0, fraction = strstr(s, "delayed") ? 0.5 : 0.0;
        unsigned k, total;
        if (*step != '\0') {
            if (strcmp(step, "half") == 0) {
                interval = 1.0;
            } else if (strcmp(step, "unison") == 0) {
                interval = 0.0;
            } else if (strcmp(step, "whole") != 0) {
                return fail(r, "invalid ornament step");
            }
        }
        if (numeric_attribute(r, ornament, "beats", count, &count) != 0 || count < 2.0 ||
            count > 1024.0 || floor(count) != count) {
            return fail(r, "ornament note count must be 2..1024");
        }
        if (strcmp(attribute(r, ornament, "accelerate"), "yes") == 0 ||
            *attribute(r, ornament, "second-beat") != '\0' ||
            *attribute(r, ornament, "last-beat") != '\0' ||
            (*attribute(r, ornament, "two-note-turn") != '\0' &&
             strcmp(attribute(r, ornament, "two-note-turn"), "none") != 0)) {
            r->score->unrendered_marks++;
        }
        total = (unsigned)count;
        if (fraction > 0.0) {
            HWAMusicXMLEvent held = *event;
            held.duration_beats *= fraction;
            held.interpretation |= HWA_MUSICXML_BASELINE_POLICY;
            if (append(r, &held, measure) != 0) {
                return -1;
            }
        }
        for (k = 0U; k < total; k++) {
            ...
            if (strstr(s, "turn") != NULL) {
                ...
                if (strcmp(start, "upper") == 0 || strcmp(start, "below") == 0) {
                    alternate = 1U - alternate;
                } else if (*start != '\0' && strcmp(start, "main") != 0) {
                    return fail(r, "invalid ornament starting note");
                }
                offset = (double)alternate * interval *
                         (strcmp(s, "mordent") == 0 || strcmp(start, "below") == 0 ? -1.0 : 1.0);
            }
            played.start_beats +=
                event->duration_beats * (fraction + (1.0 - fraction) * (double)k / (double)total);
            played.duration_beats = event->duration_beats * (1.0 - fraction) / (double)total * gate;
            played.midi_pitch += offset;
            played.interpretation |= HWA_MUSICXML_BASELINE_POLICY;
            played.sequence = k + 1U;
            if (played.midi_pitch < 0.0 || played.midi_pitch > 127.0) {
                return fail(r, "ornament pitch outside MIDI range");
            }
            if (append(r, &played, measure) != 0) {
                return -1;
            }
        }
        return 0;
...
static int ending_mask(XMLReader *r, const char *s, unsigned *mask)
{
    *mask = 0U;
    do {
        ...
        if (*s < '0' || *s > '9') {
            return fail(r, "invalid ending number");
        }
        while (*s >= '0' && *s <= '9') {
            n = n * 10U + (unsigned)(*s++ - '0');
            if (n > 32U) {
                return fail(r, "ending number exceeds 32");
            }
        }
        if (n == 0U) {
            return fail(r, "ending number must be positive");
        }
        *mask |= 1U << (n - 1U);
        while (is_xml_space((unsigned char)*s)) {
            s++;
        ...
        if (*s++ != ',') {
            return fail(r, "invalid ending number list");
        }
    } while (1);
}
...
static int navigation(XMLReader *r, size_t n, size_t ordinal, double cursor)
{
    ...
    if (strcmp(r->nodes[n].name, "barline") == 0) {
        ...
        if (repeat != 0U) {
            const char *direction = attribute(r, repeat, "direction"),
                       *times = attribute(r, repeat, "times");
            double count = 2.0;
            if (strcmp(direction, "forward") == 0) {
                if (strcmp(location, "left") != 0) {
                    return fail(r, "forward repeat must be on a left barline");
                }
                m->repeat_start = 1U;
            } else if (strcmp(direction, "backward") == 0) {
                if (*location != '\0' && strcmp(location, "right") != 0) {
                    return fail(r, "backward repeat must be on a right barline");
                }
                if (*times != '\0' && decimal(times, &count) != 0) {
                    return fail(r, "invalid repeat count");
                }
                if (count < 1.0 || count > 32.0 || floor(count) != count) {
                    return fail(r, "repeat count must be 1..32");
                }
                if (m->repeat_times != 0U && m->repeat_times != (unsigned)count) {
                    return fail(r, "parts disagree on repeat count");
                }
                m->repeat_times = (unsigned)count;
            } else {
                return fail(r, "invalid repeat direction");
            }
        }
        if (ending != 0U) {
            unsigned mask;
            const char *type = attribute(r, ending, "type");
            if (ending_mask(r, attribute(r, ending, "number"), &mask) != 0) {
                return -1;
            }
            if (strcmp(type, "start") == 0) {
                if (strcmp(location, "left") != 0) {
                    return fail(r, "ending must start on a left barline");
                }
                if (m->ending_mask != 0U && m->ending_mask != mask) {
                    return fail(r, "parts disagree on ending");
                }
                m->ending_mask = mask;
            } else if (strcmp(type, "stop") == 0 || strcmp(type, "discontinue") == 0) {
                if (*location != '\0' && strcmp(location, "right") != 0) {
                    return fail(r, "ending must stop on a right barline");
                }
                if (m->ending_stop != 0U && m->ending_stop != mask) {
                    return fail(r, "parts disagree on ending stop");
                }
                m->ending_stop = mask;
            } else {
                return fail(r, "invalid ending type");
            }
        }
    } else {
        ...
        for (j = 0U; j < 4U; j++) {
            ...
            if (child(r, sound, "offset") != 0U || child(r, n, "offset") != 0U ||
                (j < 2U && cursor != 0.0)) {
                return fail(r, "navigation signs must occur at measure boundaries without offsets");
            }
            if (*destinations[j] != NULL && strcmp(*destinations[j], label) != 0) {
                return fail(r, "parts disagree on navigation label");
            }
        ...
        if (*attribute(r, sound, "dacapo") != '\0') {
            if (strcmp(attribute(r, sound, "dacapo"), "yes") != 0) {
                return fail(r, "invalid da capo");
            }
            m->dacapo = 1;
            m->jump_at = cursor;
        }
        if (*attribute(r, sound, "fine") != '\0') {
            if (strcmp(attribute(r, sound, "fine"), "yes") != 0) {
                return fail(r, "numeric fine duration is unsupported");
            }
            m->fine = 1;
            m->jump_at = cursor;
        }
        if (strcmp(attribute(r, sound, "forward-repeat"), "yes") == 0) {
            m->repeat_start = 1U;
        }
    }
    return 0;
...
static int unfold(XMLReader *r)
{
    ...
    size_t i, active_ending = SIZE_MAX, index = 0U, depth = 0U, steps = 0U,
    ...
    for (i = 0U; i < r->measure_count; i++) {
        ...
        if (m->repeat_start || m->repeat_times || m->ending_mask || m->dacapo || m->dalsegno ||
            m->tocoda) {
            has_navigation = 1;
        }
        if (m->ending_mask) {
            if (active_ending != SIZE_MAX) {
                return fail(r, "overlapping endings");
            }
            active_ending = i;
        }
        if (m->ending_stop) {
            if (active_ending == SIZE_MAX ||
                r->measures[active_ending].ending_mask != m->ending_stop) {
                return fail(r, "unmatched ending stop");
            }
            r->measures[active_ending].ending_end = i;
            active_ending = SIZE_MAX;
        }
    }
    if (active_ending != SIZE_MAX) {
        return fail(r, "unclosed ending");
    }
    for (i = 0U; i < r->measure_count; i++) {
        if (r->measures[i].ending_mask) {
            size_t end = r->measures[i].ending_end, next_ending = end + 1U;
            unsigned mask = r->measures[i].ending_mask, highest = 1U;
            if (!r->measures[end].repeat_times) {
                continue;
            }
            ...
            if (r->measures[end].repeat_times < highest) {
                r->measures[end].repeat_times = highest;
            }
        }
    }
    if (!has_navigation) {
        if (r->measure_count > r->limits.max_measure_visits) {
            return fail(r, "measure visit limit exceeded");
        }
        r->score->measure_visits = r->measure_count;
        for (i = 0U; i < r->score->event_count; i++) {
            HWAMusicXMLEvent *e = &r->score->events[i];
            e->measure_index = r->event_measures[i];
            e->occurrence = 1U;
        }
        return 0;
    }
    original = r->score->events;
    next = r->event_measures;
    original_count = r->score->event_count;
    original_capacity = r->event_capacity;
    ...
    r->score->events = NULL;
    r->score->event_count = 0U;
    r->event_measures = NULL;
    r->event_capacity = 0U;
    while (index < r->measure_count) {
        XMLMeasure *m = &r->measures[index];
        unsigned pass;
        if (++steps > r->limits.max_measure_visits) {
            (void)fail(r, "measure visit limit exceeded");
            goto done;
        }
        if (!jumped && m->repeat_start && (depth == 0U || stack[depth - 1U].begin != index)) {
            if (depth == 64U) {
                (void)fail(r, "repeat nesting limit exceeded");
                goto done;
            }
            stack[depth].begin = index;
            stack[depth++].pass = 1U;
        }
        ...
        if (m->ending_mask && !(m->ending_mask & (1U << (pass - 1U)))) {
            ...
            if (end == SIZE_MAX) {
                (void)fail(r, "unclosed ending");
                goto done;
            }
            if (r->measures[end].repeat_times && depth != 0U) {
        ...
        if (previous_index != SIZE_MAX && previous_index + 1U != index && m->initial_tempo > 0.0) {
            ...
            restored.kind = HWA_MUSICXML_TEMPO;
            restored.start_beats = clock;
            restored.written_start_beats = m->start;
        ...
        for (i = m->head; i != SIZE_MAX; i = next[i]) {
            ...
            event.start_beats = clock + (event.start_beats - m->start);
            event.occurrence = m->visits;
            if (append(r, &event, index) != 0) {
                goto done;
            }
        }
        clock += m->length;
        if (!isfinite(clock) || clock > 1e12) {
            (void)fail(r, "expanded score timing limit exceeded");
            goto done;
        }
        if (jumped && m->fine) {
            break;
        ...
        if (!jumped && (m->dacapo || m->dalsegno)) {
            ...
            if (m->dacapo && m->dalsegno) {
                (void)fail(r, "conflicting playback jumps");
                goto done;
            }
            if (m->dalsegno) {
                for (i = 0U; i < r->measure_count; i++) {
                    if (r->measures[i].segno && strcmp(r->measures[i].segno, m->dalsegno) == 0) {
                        if (destination != SIZE_MAX) {
                            (void)fail(r, "duplicate segno label");
                            goto done;
                        }
                        destination = i;
                    }
            ...
            if (destination == SIZE_MAX || destination > index) {
                (void)fail(r, "missing or forward segno destination");
                goto done;
            }
            jumped = 1;
            last_pass = 2U;
        ...
        if (jumped && !coda_taken && m->tocoda) {
            ...
            for (i = 0U; i < r->measure_count; i++) {
                if (r->measures[i].coda && strcmp(r->measures[i].coda, m->tocoda) == 0) {
                    if (destination != SIZE_MAX) {
                        (void)fail(r, "duplicate coda label");
                        goto done;
                    }
                    destination = i;
                }
            ...
            if (destination == SIZE_MAX || destination <= index) {
                (void)fail(r, "missing or backward coda destination");
                goto done;
            }
            coda_taken = 1;
            index = destination;
        ...
        if (!jumped && m->repeat_times) {
            if (depth == 0U) {
                stack[0].begin = 0U;
                stack[0].pass = last_pass;
                depth = 1U;
            }
            if (stack[depth - 1U].pass < m->repeat_times) {
                stack[depth - 1U].pass++;
                index = stack[depth - 1U].begin;
                continue;
            }
            last_pass = stack[--depth].pass;
        } else if (depth == 0U && !m->ending_mask) {
            last_pass = 1U;
        }
        index++;
    }
    if (depth != 0U && !jumped) {
        (void)fail(r, "unclosed forward repeat");
        goto done;
    }
    r->score->duration_beats = clock;
    r->score->unfolded = 1;
    result = 0;
done:
    free(original);
...
static int read_part(XMLReader *r, size_t part, const char *part_name)
{
    ...
    for (m = r->nodes[part].first; m != 0U; m = r->nodes[m].next) {
        size_t n, first_event = r->score->event_count;
        double cursor = 0.0, source_cursor = 0.0, extent = 0.0, last_start = 0.0,
               last_duration = 0.0, source_repair_end = 0.0;
        const char *last_voice = "";
        int last_note = 0, last_grace = 0;
        if (r->nodes[m].attribute) {
            continue;
        }
        r->pos = r->nodes[m].start;
        r->measure_number = attribute(r, m, "number");
        if (strcmp(r->nodes[m].name, "measure") != 0) {
            return fail(r, "expected partwise measure");
        }
        if (*attribute(r, m, "number") == '\0') {
            return fail(r, "measure needs a number");
        }
        if (r->score->part_count != 0U && ordinal >= expected_measures) {
            return fail(r, "parts must have matching measure counts");
        }
        if (measure_slot(r, ordinal) != 0) {
            return -1;
        }
        for (n = r->nodes[m].first; n != 0U; n = r->nodes[n].next) {
            ...
            event.mark = "";
            event.grace_previous = event.grace_following = event.grace_make = -1.0;
            event.id = attribute(r, n, "id");
            event.part = part_id;
            event.part_name = part_name;
            event.voice = value(r, n, "voice", "1");
            event.staff = value(r, n, "staff", "1");
            event.measure = attribute(r, m, "number");
            event.start_beats = cursor;
            event.source_offset = r->nodes[n].start;
            event.source_size = r->nodes[n].end - r->nodes[n].start;
            if (strcmp(tag, "attributes") == 0) {
                ...
                if (unique_fields(r, n, fields, sizeof(fields) / sizeof(fields[0])) != 0) {
                    return -1;
                }
                ...
                if (time != 0U && (cursor != 0.0 || *attribute(r, time, "number") != '\0' ||
                                   meter(r, time, &bar_length) != 0)) {
                    return fail(r, "unsupported or invalid meter change");
                }
                if (trans != 0U) {
                    ...
                    if (number(r, trans, "chromatic", &chromatic) != 0 ||
                        (child(r, trans, "octave-change") != 0U &&
                         number(r, trans, "octave-change", &octave) != 0)) {
                        return -1;
                    }
                ...
                double duration;
                if (divisions <= 0.0 || number(r, n, "duration", &duration) != 0 ||
                    duration <= 0.0) {
                    return fail(r, "invalid backup/forward duration or missing divisions");
                }
                duration /= divisions;
                if (r->limits.repair_tuplets && strcmp(tag, "forward") == 0 &&
                    cursor - source_cursor > 1e-9 &&
                    fabs(duration - (cursor - source_cursor)) <= 1e-9) {
                    /* Some exporters fill the ticks lost to tuplet rounding.
                     * Exact tuplets already occupy this span. */
                    source_cursor += duration;
                    source_repair_end = source_cursor;
                    r->score->repaired_tuplet_forwards++;
                } else if (r->limits.repair_tuplets && strcmp(tag, "backup") == 0 &&
                    fabs(cursor - source_cursor) > 1e-9) {
                    if (source_cursor - cursor > 1e-9 &&
                        fabs(duration - (source_cursor - cursor)) <= 1e-9) {
                        /* Backward padding cancels ticks added by rounding. */
                        source_cursor -= duration;
                        source_repair_end = source_cursor;
                    } else if (fabs(duration - cursor) <= 1e-9 ||
                               fabs(duration - source_cursor) <= 1e-9) {
                        /* A full-voice rewind may use either duration sum. */
                        cursor = source_cursor = source_repair_end = 0.0;
                    } else if (source_cursor - duration >= source_repair_end - 1e-9) {
                        /* Unchanged notes after the last repair use the same
                         * duration on both clocks. No interpolation needed. */
                        cursor -= duration;
                        source_cursor -= duration;
                    } else {
                        return fail(r, "partial backup after repaired tuplets is unsupported");
                    }
                    r->score->repaired_tuplet_backups++;
                } else {
                    double shift = strcmp(tag, "backup") == 0 ? -duration : duration;
                    cursor += shift;
                    source_cursor += shift;
                }
                if (!isfinite(cursor) || cursor < -1e-9) {
                    return fail(r, "backup crosses the start of a measure");
                }
                if (cursor < 0.0) {
                    cursor = 0.0;
                }
                ...
                event.kind = cue    ? HWA_MUSICXML_CUE
                             : grace ? HWA_MUSICXML_GRACE
                             : rest ? HWA_MUSICXML_REST
                                    : HWA_MUSICXML_NOTE;
                if (!grace) {
                    double duration;
                    if (divisions <= 0.0 || number(r, n, "duration", &duration) != 0 ||
                    ...
                    if (repair_tuplet(r, n, divisions, duration, &event) != 0) {
                        return -1;
                    }
                } else if (child(r, n, "duration") != 0U) {
                    return fail(r, "grace notes must not contain duration");
                }
                pitch = child(r, n, "pitch");
                if (cue && rest) {
                    event.midi_pitch = -1.0;
                }
                if ((pitch != 0U) == rest || (rest && (grace || chord))) {
                    return fail(r, "note must contain one pitch or rest");
                }
                if (pitch != 0U) {
                    static const int semitones[] = {9, 11, 0, 2, 4, 5, 7};
                    const char *step = value(r, pitch, "step", "");
                    double octave, alter = 0.0;
                    if (unique_fields(r, pitch, pitch_fields,
                                      sizeof(pitch_fields) / sizeof(pitch_fields[0])) != 0) {
                        return -1;
                    }
                    if (strlen(step) != 1U || step[0] < 'A' || step[0] > 'G' ||
                        number(r, pitch, "octave", &octave) != 0 || floor(octave) != octave ||
                        octave < 0.0 || octave > 9.0 ||
                        (child(r, pitch, "alter") != 0U &&
                         number(r, pitch, "alter", &alter) != 0)) {
                        return fail(r, "invalid pitch");
                    }
                    event.midi_pitch = 12.0 * (octave + 1.0) +
                                       (double)semitones[(unsigned)(step[0] - 'A')] + alter +
                                       transpose;
                    if (event.midi_pitch < 0.0 || event.midi_pitch > 127.0) {
                        return fail(r, "sounding pitch is outside MIDI 0..127");
                    }
                }
                if (chord) {
                    if (!last_note || last_grace != grace ||
                        strcmp(last_voice, event.voice) != 0 ||
                        event.duration_beats > last_duration + 1e-9) {
                        return fail(r, "chord has no compatible preceding note");
                    }
                    event.start_beats = last_start;
                } else {
                    last_start = cursor;
                    last_duration = event.duration_beats;
                    last_voice = event.voice;
                    last_grace = grace;
                    last_note = !rest;
                    cursor += event.duration_beats;
                    source_cursor += source_duration;
                    if (fabs(event.duration_beats - source_duration) > 1e-9) {
                        source_repair_end = source_cursor;
                    }
                }
                for (tie = r->nodes[n].first; tie != 0U; tie = r->nodes[tie].next) {
                    if (!r->nodes[tie].attribute && strcmp(r->nodes[tie].name, "tie") == 0) {
                        ...
                        unsigned bit = strcmp(type, "start") == 0  ? 1U
                                       : strcmp(type, "stop") == 0 ? 2U
                                                                   : 0U;
                        if (bit == 0U || rest || cue || (event.tie & bit)) {
                            return fail(r, "invalid sound tie");
                        }
                        event.tie |= bit;
                    }
                }
                if (!isfinite(cursor) || !isfinite(event.duration_beats) ||
                    (!grace && event.duration_beats <= 0.0)) {
                    return fail(r, "note timing overflow");
                }
                if (cursor > extent) {
                    extent = cursor;
                }
                event.written_start_beats = event.start_beats;
                if (note_expression(r, &event, n, ordinal, divisions) != 0) {
                    return -1;
                }
                if (grace) {
                    r->score->grace_count++;
                ...
                event.voice = value(r, n, "voice", "");
                event.staff = value(r, n, "staff", "");
                if (navigation(r, n, ordinal, cursor) != 0) {
                    return -1;
                }
                if (unique_fields(r, n, fields, sizeof(fields) / sizeof(fields[0])) != 0) {
                    return -1;
                }
                if (direction_offset != 0U &&
                    (divisions <= 0.0 || number(r, n, "offset", &offset) != 0)) {
                    return fail(r, "invalid direction offset");
                }
                if (offset != 0.0) {
                    offset /= divisions;
                }
                event.start_beats += offset;
                if (event.start_beats < 0.0 || !isfinite(event.start_beats)) {
                    return fail(r, "direction offset crosses measure start");
                }
                ...
                if (strcmp(tag, "sound") == 0 ||
                    strcmp(attribute(r, direction_offset, "sound"), "yes") == 0) {
                    playback_start = event.start_beats;
                }
                if (sound != 0U && child(r, sound, "offset") != 0U) {
                    if (divisions <= 0.0 || number(r, sound, "offset", &offset) != 0) {
                        return fail(r, "invalid sound offset");
                    }
                    playback_start = cursor + offset / divisions;
                    if (!isfinite(playback_start) || playback_start < 0.0) {
                        return fail(r, "invalid sound position");
                    }
                }
                if (controls(r, &event, playback_start, n, ordinal) != 0) {
                    return -1;
                }
                if (bpm != 0.0) {
                    if (*attribute(r, sound, "tempo") != '\0') {
                        event.start_beats = event.written_start_beats = playback_start;
                    }
            ...
            } else if (strcmp(tag, "print") != 0 && strcmp(tag, "harmony") != 0 &&
                       strcmp(tag, "figured-bass") != 0 && strcmp(tag, "bookmark") != 0 &&
                       strcmp(tag, "link") != 0 && strcmp(tag, "grouping") != 0) {
                return fail(r, "unsupported measure content");
            }
        }
        if (strcmp(attribute(r, m, "implicit"), "yes") != 0 && bar_length > 0.0) {
            if (extent > bar_length + 1e-8 * fmax(1.0, bar_length)) {
                if (!r->limits.preserve_overfull_measures) {
                    char message[160];
                    (void)snprintf(message, sizeof(message),
                                   "measure exceeds its meter: %.17g > %.17g quarter notes",
                                   extent, bar_length);
                    r->pos = r->nodes[m].start;
                    return fail(r, message);
                }
                r->score->overfull_measures++;
                for (n = first_event; n < r->score->event_count; n++) {
                    r->score->events[n].interpretation |= HWA_MUSICXML_OVERFULL_MEASURE;
                }
            } else {
                extent = bar_length;
            }
        ...
        if (extent <= 0.0 || !isfinite(extent)) {
            return fail(r, "measure has no positive time span");
        }
        if (r->measures[ordinal].jump_at >= 0.0 &&
            fabs(r->measures[ordinal].jump_at - extent) > 1e-8) {
            return fail(r, "playback jump must follow the final measure duration");
        }
        if (extent > r->measures[ordinal].length) {
            r->measures[ordinal].length = extent;
        }
        ordinal++;
    }
    ...
    r->score->part_count++;
    r->part_id = r->measure_number = NULL;
    return 0;
}

static int read_score(XMLReader *r, size_t root)
{
    size_t part, list = child(r, root, "part-list"), i, output, last_tempo = SIZE_MAX,
                 definitions = 0U;
    double cursor = 0.0;
    ...
    for (i = r->nodes[list].first; i != 0U; i = r->nodes[i].next) {
        if (!r->nodes[i].attribute && strcmp(r->nodes[i].name, "score-part") == 0) {
            definitions++;
        }
    }
    for (part = r->nodes[root].first; part != 0U; part = r->nodes[part].next) {
        ...
        if (r->nodes[part].attribute || strcmp(r->nodes[part].name, "part") != 0) {
            continue;
        }
        if (r->score->part_count == 256U) {
            return fail(r, "part limit exceeded");
        }
        id = attribute(r, part, "id");
        if (*id == '\0') {
            return fail(r, "part needs an id");
        }
        for (previous = r->nodes[root].first; previous != part;
             previous = r->nodes[previous].next) {
            if (!r->nodes[previous].attribute && strcmp(r->nodes[previous].name, "part") == 0 &&
                strcmp(attribute(r, previous, "id"), id) == 0) {
                return fail(r, "duplicate part id");
            }
        }
        for (definition = r->nodes[list].first; definition != 0U;
             definition = r->nodes[definition].next) {
            if (!r->nodes[definition].attribute &&
                strcmp(r->nodes[definition].name, "score-part") == 0 &&
                strcmp(attribute(r, definition, "id"), id) == 0) {
                if (part_name != NULL) {
                    return fail(r, "duplicate score-part definition");
                }
                part_name = value(r, definition, "part-name", "");
            }
        }
        if (part_name == NULL) {
            return fail(r, "part id is absent from part-list");
        }
        if (read_part(r, part, part_name) != 0) {
            return -1;
        }
    }
    if (r->score->part_count == 0U || r->score->event_count == 0U) {
        return fail(r, "score has no events");
    }
    if (definitions != r->score->part_count) {
        return fail(r, "part-list and score parts do not match");
    }
    for (i = 0U; i < r->measure_count; i++) {
        r->measures[i].start = cursor;
        cursor += r->measures[i].length;
        if (!isfinite(cursor) || cursor > 1e12) {
            return fail(r, "score timing limit exceeded");
        }
    }
    ...
    qsort(r->score->events, r->score->event_count, sizeof(*r->score->events),
          hwa_musicxml_event_order);
    for (i = 0U, output = 0U; i < r->score->event_count; i++) {
        HWAMusicXMLEvent *event = &r->score->events[i];
        if (event->kind == HWA_MUSICXML_TEMPO) {
            if (last_tempo != SIZE_MAX &&
                r->score->events[last_tempo].start_beats == event->start_beats) {
                if (r->score->events[last_tempo].tempo_bpm != event->tempo_bpm) {
                    ...
                    r->score->events[last_tempo].interpretation |= HWA_MUSICXML_TEMPO_CHOICE;
    ...
    if (r->limits.default_tempo_bpm > 0.0 && (r->score->events[0].kind != HWA_MUSICXML_TEMPO ||
                                              r->score->events[0].start_beats != 0.0)) {
        HWAMusicXMLEvent event;
        memset(&event, 0, sizeof(event));
        event.kind = HWA_MUSICXML_TEMPO;
...
static int read_document(const unsigned char *data, size_t size, const HWAMusicXMLOptions *options,
                         HWAMusicXMLScore *score, const char **member, char *error,
                         size_t error_size)
{
    XMLReader r;
    size_t root = 0U;
    ...
    if (options != NULL) {
        ...
        hwa_musicxml_options_default(&r.limits);
    }
    if (score != NULL) {
    ...
    if (data == NULL || score == NULL || size == 0U || (uint64_t)size > r.limits.max_input_bytes ||
        ...
        r.limits.max_events == 0U || r.limits.max_work_bytes == 0U ||
        r.limits.max_measure_visits == 0U ||
        ...
        (r.limits.preserve_overfull_measures != 0 && r.limits.preserve_overfull_measures != 1) ||
        !isfinite(r.limits.grace_fraction) || r.limits.grace_fraction <= 0.0 ||
        r.limits.grace_fraction >= 1.0 || !isfinite(r.limits.trill_notes_per_beat) ||
        r.limits.trill_notes_per_beat < 1.0 || r.limits.trill_notes_per_beat > 128.0 ||
        !isfinite(r.limits.staccato_ratio) || r.limits.staccato_ratio <= 0.0 ||
        r.limits.staccato_ratio > 1.0 || !isfinite(r.limits.default_velocity) ||
        r.limits.default_velocity < 0.0 || r.limits.default_velocity > 127.0 ||
        !isfinite(r.limits.default_tempo_bpm) || r.limits.default_tempo_bpm < 0.0 ||
        r.limits.default_tempo_bpm > 1e6) {
        return fail(&r, "invalid input, options, or input byte limit");
    }
    if (validate_utf8(data, size) != 0) {
        return fail(&r, "expected valid UTF-8 and XML 1.0 characters");
    }
    ...
    for (;;) {
        ...
        if (starts_with(&r, "<!DOCTYPE")) {
            ...
            while (r.pos < size) {
                ...
                if (quote != 0U) {
                    ...
                    (void)fail(&r, "internal DTDs/entities are unsupported");
                    goto done;
                } else if (c == '>') {
                    break;
                }
            ...
            if (r.pos == size) {
                (void)fail(&r, "unterminated DOCTYPE");
                goto done;
            }
            continue;
        }
        if (root != 0U) {
            if (r.pos != size) {
                (void)fail(&r, "content after root element");
                goto done;
            }
            break;
        }
        if (!starts_with(&r, "<") || (root = element(&r, 0U, 1U)) == 0U) {
            if (error != NULL && error_size != 0U && error[0] == '\0') {
                (void)fail(&r, "missing root element");
            }
            goto done;
        }
    }
    if (member != NULL) {
        ...
        if (strcmp(r.nodes[root].name, "container") != 0 || entry == 0U ||
            *attribute(&r, entry, "full-path") == '\0' ||
            ...
            (void)fail(&r, "invalid MusicXML container rootfile");
            goto done;
        }
        *member = attribute(&r, entry, "full-path");
        result = 0;
    } else {
        result = read_score(&r, root);
        if (result == 0) {
            result = hwa_musicxml_perform(score, &r.limits, r.limits.max_work_bytes - r.work, error,
                                          error_size);
        }
    }
done:
...
int hwa_musicxml_read(const unsigned char *data, size_t size, const HWAMusicXMLOptions *options,
                      HWAMusicXMLScore *score, char *error, size_t error_size)
{
    ...
    if (options != NULL) {
        ...
        hwa_musicxml_options_default(&limits);
    }
    if (score != NULL) {
        memset(score, 0, sizeof(*score));
    }
    if (data == NULL || score == NULL || size == 0U || size == SIZE_MAX ||
        ...
        hwa_set_error(error, error_size,
                      "MusicXML: invalid input, input byte limit or work byte limit");
        return -1;
    }
    parse_limits = limits;
    if (size >= 2U && data[0] == 'P' && data[1] == 'K') {
        ...
        uint64_t cap = limits.max_input_bytes < limits.max_work_bytes ? limits.max_input_bytes
                                                                      : limits.max_work_bytes;
        if (hwa_musicxml_zip_member(data, size, "META-INF/container.xml", cap, &container,
                                    &container_size, error, error_size) != 0) {
            return -1;
        }
        parse_limits.max_work_bytes -= (uint64_t)container_size + 1U;
        result = read_document(container, container_size, &parse_limits, &container_tree, &member,
                               error, error_size);
        if (result == 0) {
            uint64_t occupied = (uint64_t)container_size * 3U + 2U;
            if (occupied >= limits.max_work_bytes) {
                hwa_set_error(error, error_size, "MusicXML container exceeds work byte limit");
                result = -1;
            } else {
                cap = limits.max_work_bytes - occupied;
                if (cap > limits.max_input_bytes) {
                    cap = limits.max_input_bytes;
                }
                result = hwa_musicxml_zip_member(data, size, member, cap, &xml, &xml_size, error,
                                                 error_size);
            }
        }
        hwa_musicxml_score_free(&container_tree);
        free(container);
        if (result != 0) {
            return -1;
        }
        ...
        xml = malloc(size + 1U);
        if (xml == NULL) {
            hwa_set_error(error, error_size, "MusicXML: out of memory");
            return -1;
        }
...
void hwa_musicxml_options_default(HWAMusicXMLOptions *options)
{
    ...
    options->max_events = 100000U;
    options->max_nodes = 1000000U;
    ...
    options->grace_fraction = 0.125;
    options->trill_notes_per_beat = 8.0;
    options->staccato_ratio = 0.5;
    options->default_velocity = 64.0;
}
